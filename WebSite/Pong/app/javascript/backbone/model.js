    ...
    parse: function (response) {
        if (response && response.length > 0 && Array.isArray(response)) {
            response.forEach(function (element) {
                var ret = "<div id='message'><div id='content'><div id='username'><button class='UserInformation' value='"+ element.author_id +"' > "+ element.guild +" " + element.author + " - " + element.date + "</button></div><div id='text'><p>" + element.content + "</p></div></div>";
                if (element.admin == 1) {
...
PrivateConversation = Backbone.Model.extend({
    parse: function (response) {
        if (response.length > 0 && Array.isArray(response)) {
            response.forEach(function (element) {
                var ret = "<div id='message'><div id='content'><div id='username'><button class='UserInformation' value='"+ element.author_id +"'> "+ element.guild +" " + element.author + "- " + element.date + "</button></div><div id='text'><p>" + element.content + "</p></div></div><div id='action'>";
                if (element.admin == 1) {
