class TournamentsController < ApplicationController
	...
	def create
		if (@me.role == 1)
			if (params[:tournamentname] == "" || !safestr(params[:tournamentname]) || params[:tournamentname].length > 20)
				...
				render html: "error-5";
			elsif (params[:speed].to_i < 1 || params[:speed].to_i > 3)
				render html: "error-6";
			else
				@tournament = Tournament.new;
				@status = 0;
	...
	def show
		...
		@wars_histories = History.where({tournament_id: @tournament.id, ranked: true, war_match: false}).order(:id).reverse_order
	end
